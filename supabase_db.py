...
# Save registration data (full session or account-only when course fields are empty)
def save_registration_data(user_data, start_time=None):
    try:
        ...
        # Support both student_email and email keys
        email = user_data.get("student_email") or user_data.get("email", "")
        course_name = user_data.get("course_name") or ""
        course_id = user_data.get("course_id") or ""
        professor = user_data.get("professor") or ""
        professor_email = user_data.get("professor_email") or ""
...
# Look up returning student by Student ID and email (for quick re-entry without full login)
def get_student_by_credentials(student_id, email):
    """Return the most recent registration row for this student, or None if not found."""
    try:
        supabase = init_supabase()
        if not supabase:
            return None
        ...
        if not response.data or len(response.data) == 0:
            return None
        row = response.data[0]
        # Return in user_data shape (use email for both keys for compatibility)
        return {
        ...
        st.error(f"Error looking up student: {str(e)}")
        return None
